...
def insert_vehicle(new_vehicle, new_specs):
    new_engine = find_engine(new_specs['engine'])[0] if find_engine(new_specs['engine']) else None
    if new_engine is None:
        Engine.create(engine = new_specs['engine'])
        new_engine = find_engine(new_specs['engine'])[0]
    
    created_specs = Specs.create(version = new_specs['version'], doors = new_specs['version'], seats = new_specs['seats'],
        height = new_specs['height'], width = new_specs['width'], length = new_specs['length'],
        axis_distance = new_specs['axis_distance'], tare = new_specs['tare'], gross_vehicle_weight_rating = new_specs['gross_vehicle_weight_rating'],
        tires = 'tires', engine = new_engine['id_engine'], max_potency = new_specs['max_potency'],
        wheel_drive = new_specs['wheel_drive'], urban_fuel_economy = new_specs['urban_fuel_economy'],
        extra_urban_fuel_economy = new_specs['extra_urban_fuel_economy'], overall_fuel_economy = new_specs['overall_fuel_economy'],
        addons = new_specs['addons'], notes = new_specs['notes'])
    
    found_v_type = get_dict(Vtype.select().where(Vtype.vehicle_type == new_vehicle['vehicle_type']))[0]
    found_brand = get_dict(Brand.select().where(Brand.name == new_vehicle['brand']))[0]
    found_status = get_dict(Status.select().where(Status.status == new_vehicle['status']))[0]
    ...
    new_vehicle = Vehicle.create(plate_number = new_vehicle['plate_number'], vehicle_type = found_v_type['id_type'],
                                color = new_vehicle['color'], price = new_vehicle['price'], status = found_status['id_status'],
                                img_url = new_vehicle['img_url'], model = new_model['id_model'], specs = created_specs.id_specs)
...
def delete_vehicle(v_id):
    specs_to_delete = get_dict(Specs.select().join(Vehicle).where(Specs.id_specs == Vehicle.specs, Vehicle.plate_number == v_id))[0]
